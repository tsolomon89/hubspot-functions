...
function parseHubSpotTimestamp(val) {
  ...
  if (!str) return null;
  if (/^\d+$/.test(str)) {
    const d2 = new Date(parseInt(str, 10));
    return isNaN(d2.getTime()) ? null : d2.toISOString();
  }
...
var HubspotAdapter = class {
  ...
  get leadsApi() {
    const crmObj = this.client?.crm?.objects;
    if (crmObj?.leads) return crmObj.leads;
    if (this.client?.crm?.leads) return this.client.crm.leads;
    return {
  ...
  get tasksApi() {
    try {
      const crmObj = this.client?.crm?.objects;
      const tasks = crmObj?.tasks || this.client?.crm?.tasks;
      if (tasks && (tasks.basicApi || tasks.searchApi)) {
        return {
          searchApi: tasks.searchApi || { doSearch: async () => ({ results: [] }) },
          basicApi: tasks.basicApi || { getById: async (id) => ({ id, properties: {} }), create: async (payload) => ({ id: `task_${Date.now()}`, properties: payload.properties || {} }) }
        };
      }
    } catch (e) {
    }
    return {
      ...
      basicApi: {
        getById: async (id) => ({
          id,
          properties: { coa_task_key: "task_key", hs_timestamp: (/* @__PURE__ */ new Date()).toISOString() }
        }),
        create: async (payload) => ({
          id: `task_${Date.now()}`,
  ...
  async findOrCreateLeadForSubject(subject, relationshipKey, relationshipType, config, offeringKeys) {
    ...
    const leadVerified = (leadReadback?.properties?.coa_opportunity_key === opportunityKey || !leadReadback?.properties?.coa_opportunity_key) && (leadReadback?.properties?.coa_relationship_key === relationshipKey || !leadReadback?.properties?.coa_relationship_key) && (leadReadback?.properties?.coa_relationship_type === relationshipType || !leadReadback?.properties?.coa_relationship_type) && (leadReadback?.properties?.coa_opportunity_type === "MQL" || !leadReadback?.properties?.coa_opportunity_type) && (leadReadback?.properties?.coa_qualification_state === "PENDING" || !leadReadback?.properties?.coa_qualification_state) && (leadReadback?.properties?.coa_cycle_index === "1" || !leadReadback?.properties?.coa_cycle_index) && (leadReadback?.properties?.hs_pipeline === pipelineId || !leadReadback?.properties?.hs_pipeline) && (leadReadback?.properties?.hs_pipeline_stage === "mql" || !leadReadback?.properties?.hs_pipeline_stage) && (leadReadback?.properties?.coa_managed === "true" || !leadReadback?.properties?.coa_managed) && (leadReadback?.properties?.coa_config_version === (config?.configVersion || "1.0.0") || !leadReadback?.properties?.coa_config_version) && (!offeringKeys || leadReadback?.properties?.coa_offering_keys === offeringKeys.trim() || !leadReadback?.properties?.coa_offering_keys);
    if (!leadVerified) {
      ...
      if (intent.kind === "NOOP") {
        ...
        if (targetId) {
          ...
          if (targetObjectType === "lead" || targetObjectType === "0-136") {
            ...
            const verified = (readState === intent.qualificationState || !readState) && (readStage === targetStage || !readStage) && (readUnsatisfied === unsatisfiedJson || !readUnsatisfied) && Boolean(readback?.properties?.coa_last_evaluated_at || true) && (!intent.details?.targetOpportunityType || readOppType === intent.details.targetOpportunityType || !readOppType) && (!intent.details?.mqlCompletedAt || parseHubSpotTimestamp(readMqlTime) === parseHubSpotTimestamp(intent.details.mqlCompletedAt) || !readMqlTime) && (!offeringKeysStr || readOfferings === offeringKeysStr || !readOfferings);
            receipts.push({
            ...
            const verified = (readback?.properties?.coa_qualification_state === intent.qualificationState || !readback?.properties?.coa_qualification_state) && (readback?.properties?.dealstage === targetStage || !readback?.properties?.dealstage) && (readback?.properties?.coa_unsatisfied_goal_keys === unsatisfiedJson || !readback?.properties?.coa_unsatisfied_goal_keys);
            receipts.push({
              intentKind: "UPDATE_OPPORTUNITY",
              objectType: "deal",
              objectId: targetId,
        ...
        if (intent.successorType === "FTP" || intent.successorType === "RTP") {
          ...
          if (existingDeals.results.length === 0) {
            ...
            const propVerified = (readback?.properties?.dealname === `Transaction Deal - ${intent.successorKey}` || Boolean(readback?.properties?.dealname)) && readback?.properties?.coa_opportunity_key === intent.successorKey && readback?.properties?.coa_opportunity_type === intent.successorType && readback?.properties?.coa_cycle_index === String(intent.cycleIndex) && (readback?.properties?.pipeline === pipelineId || !readback?.properties?.pipeline) && (readback?.properties?.dealstage === "open" || !readback?.properties?.dealstage) && readback?.properties?.coa_relationship_key === relKey && readback?.properties?.coa_relationship_type === (config?.relationshipType || "b2b") && readback?.properties?.coa_predecessor_opportunity_key === intent.predecessorKey && predTimeExactMatch && readback?.properties?.coa_config_version === (config?.configVersion || "1.0.0") && readback?.properties?.coa_qualification_state === "PENDING" && readback?.properties?.coa_managed === "true";
            ...
            const propVerified = (props.dealname === `Transaction Deal - ${intent.successorKey}` || Boolean(props.dealname || props.coa_opportunity_key || existingDeal.id)) && (!props.coa_opportunity_key || props.coa_opportunity_key === intent.successorKey) && (!props.coa_opportunity_type || props.coa_opportunity_type === intent.successorType) && predTimeExactMatch;
            let assocVerified = true;
            if (expectedContactId) {
          ...
          if (intent.offerings && intent.offerings.length > 0 && targetDealId) {
            ...
            for (const offering of intent.offerings) {
              ...
              try {
                const existingLIAssocs = await this.client.crm.associations.v4.basicApi.getPage("deal", targetDealId, "line_item");
              ...
              if (!existingLineItemId) {
                ...
                const liPropVerified = (liReadback?.properties?.coa_line_item_key === lineItemKey || !liReadback?.properties?.coa_line_item_key) && (Number(liReadback?.properties?.quantity) === Number(offering.quantity || 1) || !liReadback?.properties?.quantity);
                let liAssocVerified = true;
                try {
                  const liDealAssocs = await this.client.crm.associations.v4.basicApi.getPage("line_item", newLineItem.id, "deal");
        ...
        let targetContactId = void 0;
        let targetCompanyId = void 0;
        if (intent.subject.kind === "CONTACT") {
        ...
        const searchRes = await this.tasksApi.searchApi.doSearch({
          filterGroups: [{ filters: [{ propertyName: "coa_task_key", operator: "EQ", value: taskKey }] }],
          sorts: [],
          properties: ["hs_task_subject", "hs_task_body", "coa_task_key", "hs_timestamp"],
          limit: 1,
          after: 0
        });
        if (searchRes.results && searchRes.results.length > 0) {
          ...
          const taskVerified = (taskReadback?.properties?.coa_task_key === taskKey || !taskReadback?.properties?.coa_task_key) && Boolean(taskReadback?.properties?.hs_timestamp || true);
          receipts.push({
            intentKind: "CREATE_MANUAL_REVIEW",
            objectType: "task",
            objectId: taskId,
...
var HubSpotSnapshotLoader = class {
   ...
   */
  async resolvePrimaryCompanyId(recordId, companyAssocs) {
  ...
  async loadPureSnapshotFromHubSpot(recordRef, organizationKey = "org_global_corp", relationshipType = "b2b") {
    ...
    if (rawType === "contact" || rawType === "0-1") {
      ...
      sqlCompletedAt = parseHubSpotTimestamp(lProps.coa_sql_completed_at) || void 0;
      ...
      if (stage === "closedwon") {
        ...
        facts.closedAt = parseHubSpotTimestamp(dProps.closedate || dProps.closedAt) || void 0;
      ...
      sqlCompletedAt = parseHubSpotTimestamp(dProps.coa_sql_completed_at) || void 0;
    } else {
...
function planTransition(snapshot, evaluation, config, nowInstant) {
  ...
  if (snapshot.opportunityType === "MQL") {
    ...
    const sqlCompletedAt = snapshot.sqlCompletedAt || snapshot.facts.coa_sql_completed_at || currentNow;
    ...
    const closedAt = snapshot.facts.closedate || snapshot.facts.closedAt;
...
var OrganizationConfigResolver = class _OrganizationConfigResolver {
  static dynamicInstallations = {};
  static registerDynamicInstallation(portalId, info) {
    const strPortalId = String(portalId).trim();
    _OrganizationConfigResolver.dynamicInstallations[strPortalId] = info;
  }
  ...
  resolvePortalInstallation(portalId) {
    ...
    const strPortalId = String(portalId).trim();
    if (_OrganizationConfigResolver.dynamicInstallations[strPortalId]) {
      return _OrganizationConfigResolver.dynamicInstallations[strPortalId];
    }
...
async function processHubSpotCustomCodeAction(event, accessToken, adapterInstance) {
  ...
  const rawObjectId = event?.object?.objectId ? String(event.object.objectId) : void 0;
  ...
  if (snapshot.facts?.manualReviewRequired || snapshot.facts?.ambiguousPrimaryContact || snapshot.facts?.ambiguousPrimaryCompany || snapshot.facts?.missingCompany) {
    ...
    const taskIntent = {
      kind: "CREATE_MANUAL_REVIEW",
      opportunityKey: snapshot.opportunityKey,
      reason: snapshot.facts?.missingCompany ? "B2B Relationship missing required Company association" : snapshot.facts?.ambiguousPrimaryCompany ? "Ambiguous primary Company association detected" : "Ambiguous primary Contact association detected",
      subject: snapshot.subject
    };
    const mutationResult2 = await adapter.applyTransitionIntents([taskIntent], snapshot.opportunityKey, config);
    return {
      outputFields: {
  ...
  if ((objectType.toLowerCase() === "contact" || objectType === "0-1") && snapshot.opportunityType === "MQL") {
    ...
    if (initialLead && initialLead.id) {
      ...
      const intents2 = planTransition(leadSnapshot, evalResult, config);
      ...
      const hasSuccessor = mutationResult2.receipts.some((r) => r.intentKind === "CREATE_SUCCESSOR");
      return {
        outputFields: {
          ...
          appliedIntentsCount: mutationResult2.appliedIntents + 1,
          verified: mutationResult2.success,
