    ...
    for (const intent of intents) {
      if (intent.kind === "UPDATE_OPPORTUNITY") {
        ...
        if (targetId && targetType) {
          const targetStage = intent.details?.targetLeadStage ? String(intent.details.targetLeadStage) : intent.newState === "WON" ? targetType === "lead" ? "qualified" : "closedwon" : "sql";
          const updateProps = {
        ...
        if (intent.successorType === "SQL") {
          ...
          if (existingLeads.results.length === 0) {
            ...
            const verified = readback?.properties?.coa_opportunity_key === intent.successorKey && readback?.properties?.coa_opportunity_type === "SQL" && readback?.properties?.coa_cycle_index === String(intent.cycleIndex);
            receipts.push({
...
var HubSpotSnapshotLoader = class {
  ...
  async loadSnapshotFromRecord(recordRef, organizationKey = "org_default", relationshipType = "b2b") {
    ...
    if (recordRef.objectType === "contact") {
      ...
      relationshipKey = contactFacts.relationshipKey || contactFacts.email || `cnt_${recordRef.objectId}`;
      ...
      relationshipKey = companyFacts.relationshipKey || companyFacts.domain || `comp_${recordRef.objectId}`;
      opportunityKey = `${relationshipKey}::MQL::1`;
      ...
      opportunityKey = leadProps.coa_opportunity_key || `${relationshipKey}::${opportunityType}::${cycleIndex}`;
