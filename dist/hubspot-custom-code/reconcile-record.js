...
function evaluatePredicate(goal, snapshot) {
  ...
  const predicatesList = goal.predicates && goal.predicates.length > 0 ? goal.predicates.map((p) => ({ predicate: p.predicate, params: { ...goal.params, ...p, equals: p.value ?? goal.params?.equals } })) : [{ predicate: goal.predicate || "property", params: goal.params }];
...
var HubspotAdapter = class {
    ...
    try {
      ...
      const newLead = await this.leadsApi.basicApi.create({
        properties: {
          ...
          coa_cycle_index: "1",
          coa_managed: "true",
          coa_offering_keys: offeringKeys || "prod_software",
  ...
  async applyTransitionIntents(intents, correlationKey, config) {
    ...
    for (const intent of intents) {
      ...
      if (intent.kind === "PROJECT_LIFECYCLE_STAGE") {
        logger.info("Applying PROJECT_LIFECYCLE_STAGE intent", { stage: intent.stage });
        try {
          const targetKind = intent.subject.kind;
          const targetKey = intent.subject.key;
          if (targetKind === "CONTACT") {
            await this.client.crm.contacts.basicApi.update(targetKey, {
              properties: { lifecyclestage: intent.stage }
            });
            const readback = await this.client.crm.contacts.basicApi.getById(targetKey, ["lifecyclestage"]);
          ...
          } else if (targetKind === "COMPANY") {
            await this.client.crm.companies.basicApi.update(targetKey, {
              properties: { lifecyclestage: intent.stage }
            });
            const readback = await this.client.crm.companies.basicApi.getById(targetKey, ["lifecyclestage"]);
            const verified = readback?.properties?.lifecyclestage === intent.stage;
            receipts.push({
              intentKind: "PROJECT_LIFECYCLE_STAGE",
              objectType: "company",
          ...
          logger.error("Failed to project lifecycle stage", err);
          receipts.push({
            intentKind: "PROJECT_LIFECYCLE_STAGE",
            objectType: intent.subject.kind.toLowerCase(),
            objectId: intent.subject.key,
            operation: "UPDATE",
            verified: false,
...
var SchemaTool = class {
  ...
  plan(currentAccountSchema) {
    const manifest = this.loadManifest();
    const propertyGroupsToCreate = [];
    const propertiesToCreate = [];
    const associationLabelsToCreate = [];
    const pipelinesToCreate = [];
    const pipelinesToUpdate = [];
    const hasAnyCustomProperty = currentAccountSchema?.properties && Object.values(currentAccountSchema.properties).some(
      (arr) => Array.isArray(arr) && arr.some((p) => p.name && p.name.startsWith("coa_"))
    );
    if (!currentAccountSchema || !hasAnyCustomProperty) {
      ...
      for (const [objType, props] of Object.entries(manifest.properties || {})) {
        ...
        for (const p of props) {
          const existing = existingProps.find((e) => e.name === p.name);
      ...
      for (const [objType, pipes] of Object.entries(manifest.pipelines || {})) {
        ...
        for (const pipe of pipes) {
          const existing = existingPipes.find((e) => e.id === pipe.pipelineId || e.label === pipe.name);
          if (!existing) {
            pipelinesToCreate.push({ objectType: objType, ...pipe });
          } else {
            ...
            for (const mStage of manifestStages) {
              const eStage = existingStages.find((s) => (s.id || s.stageId) === mStage.stageId);
              if (!eStage) {
                stageDiff = true;
                break;
              }
              if (mStage.displayOrder !== void 0 && eStage.displayOrder !== void 0 && Number(mStage.displayOrder) !== Number(eStage.displayOrder)) {
                stageDiff = true;
                break;
              }
              if (mStage.metadata?.probability !== void 0 && eStage.metadata?.probability !== void 0 && String(mStage.metadata.probability) !== String(eStage.metadata.probability)) {
                stageDiff = true;
                break;
              }
            }
  ...
  async readback(hsAdapter) {
    const inspected = await this.inspect(hsAdapter);
    const diffAfterApply = this.plan(inspected);
    const verified = diffAfterApply.propertyGroupsToCreate.length === 0 && diffAfterApply.propertiesToCreate.length === 0 && diffAfterApply.pipelinesToCreate.length === 0 && diffAfterApply.pipelinesToUpdate.length === 0;
    return {
      verified,
      diffAfterApply
    };
...
if (require.main === module) {
  ...
  const hsAdapter = token ? new HubspotAdapter(token) : void 0;
  if (mode === "inspect") {
