...
import { validateConfigurations } from './validate-configs';

function compileConfigs() {
  const validation = validateConfigurations();
  if (!validation.valid) {
    console.error('❌ Refusing to compile configurations due to validation errors:');
...
export const EMBEDDED_INSTALLATIONS: Record<string, { executionPortalId?: number; accountRole?: string; organizationKey: string; allowedRelationshipTypes?: string[]; defaultRelationshipType?: string; expectedConfigVersion?: string }> = ${JSON.stringify(installations, null, 2)};
